...
|`SeRestore`| ***Admin*** | ***Built-in commands*** | 1. Launch cmd.exe with the SeRestore privilege present.<br>2. Enable the privilege (i.e. with [GrantPrivs](https://github.com/gtworek/GrantPrivs)).<br>3. Rename utilman.exe to utilman.old<br>4. Rename cmd.exe to utilman.exe<br>5. Lock the console and press Win+U| Attack may be detected by Windows Defender.<br> <br>Alternative method rely on replacing service binaries stored in "Program Files". |
